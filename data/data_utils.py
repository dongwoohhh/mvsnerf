...
def random_crop(rgb, camera, src_rgbs, src_cameras, size=(400, 600), center=None):
    ...
    #print(h, w, out_h // 2 + 1, h - out_h // 2 - 1, out_w // 2 + 1, w - out_w // 2 - 1)
    if center is not None:
        center_h, center_w = center
    else:
        center_h = np.random.randint(low=out_h // 2, high=h - out_h // 2 - 1)
        center_w = np.random.randint(low=out_w // 2, high=w - out_w // 2 - 1)
...
def preprocess_warped_depths(ray_batch):
    depths = ray_batch['warped_depths_image']
    device = depths.device
    n_views = depths.shape[1]

    weight = torch.zeros(9, 9, device=device).float()
    weight[torch.arange(9), torch.arange(9)] = 1.0
    weight = weight.reshape(9, 1, 3, 3)
    weight = weight[[0,1,2,3,5,6,7,8], :, :, :]
    
    depths = depths.squeeze(0)
    depths_filled = depths.clone()
    """
    for i in range(10):
        depth_vis = depths / torch.max(depths)
        depths_vis = depth_vis.cpu()
        imageio.imwrite('./warping_test/depth_{}_a.png'.format(i), depths_vis[i,0])
    """
    def layer1(depths):
        binary_mask = torch.where(depths==0, torch.zeros_like(depths), torch.ones_like(depths))

        depths_avg = F.avg_pool2d(depths, kernel_size=3, stride=1, padding=1)
        binary_mask_avg = F.avg_pool2d(binary_mask, kernel_size=3, stride=1, padding=1)

        depths_avg = depths_avg / binary_mask_avg
        depths_filled = torch.where(torch.logical_and(binary_mask==0, binary_mask_avg > 0.3), depths_avg, depths)

        depths_filled[binary_mask_avg <=0.3] = 0.0
        depths_filled[torch.isnan(depths_filled)] = 0.0
        
        return depths_filled

    def layer2(depths_filled, weight):
        depths_neighbor = F.conv2d(depths_filled, weight=weight, padding=1)

        depths_neighbor_sorted, _ = torch.sort(depths_neighbor, dim=1)

        depths_neighbor_min = torch.mean(depths_neighbor_sorted[:, :3], dim=1, keepdim=True)
        depths_neighbor_max = torch.mean(depths_neighbor_sorted[:, -3:], dim=1, keepdim=True)
        depths_neighbor_middle = torch.mean(depths_neighbor_sorted[:, 3:-3], dim=1, keepdim=True)

        mask_overlap = torch.logical_or(depths_neighbor_min > depths_filled, depths_neighbor_max < depths_filled)

        mask_overlap[:, :, [0, -1], :] = 0
        mask_overlap[:, :, :, [0, -1]] = 0
        
        depths_filled[mask_overlap] = depths_neighbor_middle[mask_overlap]

        return depths_filled
    
    for i in range(5):
        depths_filled = layer1(depths_filled)
        """
        for j in range(10):
            depth_vis = depths_filled / torch.max(depths_filled)
            depths_vis = depth_vis.cpu()
            imageio.imwrite('./warping_test/depth_{}_filled{}_1.png'.format(j, i), depths_vis[j,0])
        """
        depths_filled = layer2(depths_filled, weight)
        """
        for j in range(10):
            depth_vis = depths_filled / torch.max(depths_filled)
            depths_vis = depth_vis.cpu()
            imageio.imwrite('./warping_test/depth_{}_filled{}_2.png'.format(j, i), depths_vis[j,0])
        """
    #import pdb;pdb.set_trace()
    
    depths_warped_median, _ = torch.median(depths_filled, dim=0, keepdim=True)
    
    error_map = torch.abs(depths_warped_median - depths_filled) / (depths_warped_median + 1e-7)
    error_map, _ = torch.topk(error_map, k=min(5, n_views), dim=0)
    depth_consistency = error_map[1:5].mean(dim=0, keepdim=True)
    
    depths_filled = depths_filled.unsqueeze(0)
    
    """
    depth_vis = depths_warped_median / torch.max(depths_warped_median)
    depths_vis = depth_vis.cpu()
    imageio.imwrite('tmpdepth_filled3.png', depths_vis[0,0])
    """
    ray_batch.update(
        {'warped_depths_image': depths_filled,
         'depth_image': depths_warped_median,
         'depth_consistency_image': depth_consistency,
        })

    return ray_batch
    #return  depths_filled, depths_warped_median
    
def ray_sample_depths(ray_batch, indices):
    warped_depths = ray_batch['warped_depths_image']
    depth = ray_batch['depth_image']
    depth_consistency = ray_batch['depth_consistency_image']

    n_views = warped_depths.shape[1]

    warped_depths = warped_depths.reshape(n_views, -1)
    depth = depth.reshape(-1)
    depth_consistency = depth_consistency.reshape(-1)
    if indices is not None:
        ray_batch.update(
            {
                'warped_depths': warped_depths[:, indices],
                'depth': depth[indices],
                'depth_consistency': depth_consistency[indices]
            }
        )
    else:
        ray_batch.update(
