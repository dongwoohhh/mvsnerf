...
for i_scene, scene in enumerate(['ship','mic','chair','lego','drums','ficus','materials','hotdog']):#
    psnr,ssim,LPIPS_vgg = [],[],[]
    cmd = f'--datadir /media/hdd1/Datasets/MVSNeRF_data/mvs_training/nerf_synthetic/{scene}  \
    ...
    dataset_val = dataset_dict[args.dataset_name](args, split='test')
    val_idx = dataset_val.img_idx
    
    save_as_image = True
    save_dir = '/media/hdd1/results_nerf/mvsnerf/eval/nerf_synthetic/{}'.format(scene)
    save_txt_dir = '/media/hdd1/results_nerf/mvsnerf/eval/nerf_synthetic'
