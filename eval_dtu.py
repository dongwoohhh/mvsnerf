...
for i_scene, scene in enumerate([8,21,30,31,34,38,40,41,45,55,63,82,103,110,114]):#,8,21,103,114
    psnr,ssim,LPIPS_vgg = [],[],[]
    cmd = f'--datadir /media/hdd1/Datasets/MVSNeRF_data/mvs_training/dtu/scan{scene}  \
     --dataset_name dtu_ft  \
    ...
    save_as_image = True
    save_dir = '/media/hdd1/results_nerf/mvsnerf/eval/dtu/{}'.format(scene)
    save_txt_dir = '/media/hdd1/results_nerf/mvsnerf/eval/dtu'
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(tqdm(dataset_val)):
            ...
            depth_gt, _ =  read_depth(f'/media/hdd1/Datasets/MVSNeRF_data/mvs_training/dtu/Depths/scan{scene}/depth_map_{val_idx[i]:04d}.pfm')
        ...
        f = open(f"{save_txt_dir}/psnr_{scene}.txt", "a")
